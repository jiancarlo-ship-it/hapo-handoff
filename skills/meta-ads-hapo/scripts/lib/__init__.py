...
# Garante UTF-8 no stdout/stderr independente do terminal (resolve emojis em nomes de campanha)
...
_ENV_SEARCH_PATHS = [
    os.path.expanduser("~/.claude/skills/meta-ads-hapo/.env"),
...
def init_api():
    ...
    if not token:
        ...
        print("  Crie o arquivo ~/.claude/skills/meta-ads-hapo/.env com:", file=sys.stderr)
        print('  META_ADS_TOKEN="seu-token-aqui"', file=sys.stderr)
        print("", file=sys.stderr)
        print("  Ou rode: /meta-ads-hapo setup", file=sys.stderr)
        sys.exit(1)
