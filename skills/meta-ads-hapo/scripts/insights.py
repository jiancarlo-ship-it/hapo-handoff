...
# Campos de nome injetados automaticamente por nivel
_LEVEL_NAME_FIELDS = {
...
def _resolve_fields(args):
    ...
    fields = list(DEFAULT_INSIGHTS_FIELDS)
    # Inject name/id fields based on --level or subcommand (campaign/adset/ad)
    effective_level = getattr(args, "level", None) or getattr(args, "command", None)
    for f in reversed(_LEVEL_NAME_FIELDS.get(effective_level, [])):
        if f not in fields:
            fields.insert(0, f)
    return fields
